...
BEGIN
    ...
    WHERE table_name = 'ticket' AND column_name = 'user_id';

    -- 如果ticket表存在且user_id是bigint，则重建表
    IF v_user_id_type = 'bigint' THEN
        RAISE NOTICE 'ticket表user_id类型为bigint，正在重建...';
        ...
        RAISE NOTICE 'ticket和user_feedback表重建完成，user_id类型已改为UUID';
    ELSE
        RAISE NOTICE 'ticket表user_id类型已经是UUID，无需修改';
    END IF;
END $$;
