...
# -*- coding: utf-8 -*-
"""
通过Docker容器修复 storage_configs 表的 bucket_access_type CHECK 约束
"""
...
def fix_db_via_docker():
    ...
    try:
        ...
        print("=" * 60)
        print("通过Docker执行SQL修复约束...")
        print("=" * 60)

        # 通过 docker exec 执行 SQL
        cmd = f"""docker exec -i 1Panel-postgresql-9ISB psql -U user_N7TYjT -d rewind_db -c "{sql_commands.replace('"', '\\"')}" """
